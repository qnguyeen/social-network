...
const VerifyEmail = ({
  ...
  useEffect(() => {
    if (isSuccess) {
      if (data?.data?.code === 1000) {
        message.success(t(data?.data?.message));
      }
    }
  }, [isSuccess]);
  ...
  return (
    <>
      ...
      <CustomModal
        className={`${
          isMobile ? "w-full max-w-sm p-5" : "max-w-xl p-8"
        } text-center text-gray-800  cursor-default bg-primary shadow-xl lg:max-w-3xl rounded-3xl lg:p-12`}
        isOpen={isOpenVerifyEmail}
        onClose={handleCloseVerifyEmail}
      >
        <h3 className={`${isMobile ? "text-xl" : "text-2xl"} text-ascent-1`}>
          {t("Cảm ơn bạn đã đăng ký LinkVerse")}
        </h3>
        ...
        {!userDetail?.emailVerified && (
          <p className={`text-ascent-1 ${isMobile ? "text-sm" : ""}`}>
            {t(
        ...
        <div className="mt-4">
          {userDetail?.emailVerified ? (
            <span className="font-semibold leading-relaxed text-lg">
            ...
            <>
              <div className="relative flex items-center justify-center">
                <Button
                  ...
                  isLoading={isPending}
                  title={t("Bấm để xác minh email")}
                  className={`${
                    isSuccess && "hidden"
                  } px-4 py-2 text-ascent-3 hover:opacity-90 active:scale-90 hover:scale-105 transition-transform bg-bgStandard rounded-xl ${
                    isMobile ? "text-sm" : ""
                  }`}
                />
                {isSuccess && (
                  <span
                    className={`text-center text-bluePrimary ${
                      isMobile ? "text-sm" : ""
                    }`}
                  >
              ...
              >
                ...
                <a
                  ...
                  className={`text-blue-700 ${
                    isMobile ? "block mt-2 break-words" : ""
                  }`}
                >
