...
import CreateAdPost from "~/components/CreateAdPost";
...
const MyPostCard = ({
  ...
  const { isCopied: isCopiedPostId, copyToClipboard: copyToClipboardPostId } =
    useCopyToClipboard();
  ...
  const handleCloseAdModal = useCallback(() => setOpenAdModal(false));
  ...
  const [isConfirmCloseAd, setIsConfirmCloseAd] = useState(false);
  const handleCloseConfirmDelete = () => setIsConfirmDelete(false);
  const handleCloseConfirmAd = () => setIsConfirmCloseAd(false);
  ...
  useEffect(() => {
    ...
    if (isPending) {
      ...
      message.open({
        type: "success",
        content: t("Delete post successfully"),
      });
    }
  ...
  const handleOpenConfirmCloseAd = () => {
    setIsConfirmCloseAd(true);
    handleClose();
  };
  ...
  const handleDeletePost = async () => {
    ...
    try {
      ...
      if (res?.code === 200) {
        message.success({ content: t("Delete post successfully") });
        handleRefetch(true);
      }
    } finally {
  ...
  const handleCloseAd = async () => {
    ...
    try {
      if (adData?.id) {
        const res = await AdService.closeAds({ id: adData?.id });
        if (res?.code === 200) {
  ...
  return (
    <div className="bg-primary rounded-xl py-3">
      ...
      <CreateAdPost
        open={openAdModal}
        handleClose={handleCloseAdModal}
        post={post}
      />
      <SharePost
        open={openSharePost}
      ...
      />
      <ConfirmDialog
        title={t("Đóng quảng cáo")}
        confirmText={t("Đóng")}
        variant="danger"
        description={t("Bạn có chắc chắn muốn đóng quảng cáo này không?")}
        open={isConfirmCloseAd}
        loading={loadingCloseAd}
        onClose={handleCloseConfirmAd}
        onConfirm={handleCloseAd}
      />
                ...
                {userState?.token && (
                  <div>
                    ...
                    {isAdActive && (
                      <MenuItem
                        disableRipple
                        onClick={() => handleOpenConfirmCloseAd()}
                      >
                        <div className="flex items-center justify-between w-full">
                          <span className="text-red-600">{t("Close Ad")}</span>
                    ...
                    {post?.imageUrls &&
                      ...
                      !isAdActive && (
                        <MenuItem
                          onClick={handleOpenModalCreateAd}
                          disableRipple
                        >
                          <div className="flex items-center justify-between w-full">
                            <span
                    ...
                    >
                      <div className="flex items-center justify-between w-full">
                        <span className="text-red-600">{t("Hiển thị")}</span>
                        <FaRegEdit color="red" />
                      </div>
                    </MenuItem>
                  </div>
