...
const ReplyPostCard = ({ post, handleRefetch, userDetail }) => {
  ...
  const handleSavePostId = useCallback(
    (id) => {
      copyToClipboardPostId(`${id}`);
    },
    [copyToClipboardPostId]
  );
  ...
  const handleSavePost = useCallback(async (id) => {
    ...
    if (res?.code === 200) {
      message.success({ content: "Save post success!" });
      setSave(true);
      ...
      >
        ...
        <div className="w-[50px] h-[50px] border border-borderNewFeed rounded-full overflow-hidden shadow-newFeed flex-shrink-0">
          <img
            ...
            src={userDetail?.imageUrl || BlankAvatar}
            alt="User Avatar"
      ...
      <div className="w-full px-5 flex items-center justify-between">
        ...
        >
          {isSave ? (
            <FiBookmark
              size={20}
              className="transition-colors fill-zinc-400 stroke-zinc-400 duration-300"
            />
          ) : (
            <FiBookmark
              size={20}
